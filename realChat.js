...
function renderMessages(messages, activeFriend) {
  ...
  messages.forEach((msg) => {
    ...
    if (msg.is_read) {
      messageElement.style.backgroundColor = "#b0ffb9";
    } else {
      messageElement.style.backgroundColor = "#ffc9bf";
    }
...
socket.on("görüldü", async (data) => {

  if (data == "görüldü") {
    console.log("Görüldü");

  } else if (data == "çevrimiçi") {
...
socket.on(`chat${username}`, (message) => {
  ...
  socket.emit("görüldü", { sender: username, reciever: activeFriend2 });
...
socket.on("private_message", (data) => {
  activeChat.push(data);

  if (data.sender_id.username == activeFriend2) {
    socket.emit("görüldü", {
      sender: data.sender_id.username,
      content: "görüldü",
    });
