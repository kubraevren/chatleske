...
io.on("connection", (socket) => {
  socket.on("görüldü", (data) => {    

    const { sender, content } = data;

    const recipientSocketId = userSockets[sender];

    console.log("real content",content);
    
    
      io.to(recipientSocketId).emit("görüldü", content);
  ...
  socket.on("private_message", (data) => {
    ...
    const { id,sender_id,content, receiver_id,timestamp  } = data;
    console.log("Alıcı:", receiver_id.username);
    console.log("Mesaj:", content);
    const recipientSocketId = userSockets[receiver_id.username];
    ...
    if (recipientSocketId) {
      ...
      socket.emit('görüldü',('çevrimdışı'));
    }
  });
...
module.exports = {setIO,userSockets};
